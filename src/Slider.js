...
const Slider = React.createClass({
  ...
  componentWillReceiveProps: function(nextProps) {
    const oldLeftValue = this.props.value;
    const oldRightValue = this.props.rightValue;
    const newLeftValue = nextProps.value;
    const newRightValue = nextProps.rightValue;

    if (oldLeftValue !== newLeftValue || oldRightValue !== newRightValue) {
      if (this.props.animateTransitions) {
        this._setCurrentValueAnimated(newLeftValue, newRightValue);
      }
      else {
        this._setCurrentValue(newLeftValue, newRightValue);
      }
    }
  },
  ...
  _createWrappedThumbView(view) {
    ...
    return thumbComponent ? thumbComponent(view) : view;
  },
  ...
  _handlePanResponderGrant: function(e: Object, /*gestureState: Object*/) {
    ...
    const leftThumbTouchRect = this._getThumbTouchRect(this._getThumb(this._getLeftThumbValue()));
  ...
  _handlePanResponderMove: function(e: Object, gestureState: Object) {
    ...
    const buffer = this.state.thumbSize.width / this.state.trackSize.width * (this.props.maximumValue - this.props.minimumValue);

    if (this.isLeftThumb) {
      const newLeft = this._getValue(this._previousLeft, gestureState);
      const rightThumbValue = this._getRightThumbValue();
      if (this.props.multiTouch) {
        if (newLeft < rightThumbValue - buffer) {
      ...
      const leftThumbValue = this._getLeftThumbValue();
      if (this.props.multiTouch && newRight > leftThumbValue + buffer) {
  ...
  _setCurrentValue(value: number, rightValue: number) {
    if (value) {
      this.state.value.setValue(parseFloat(value.toFixed(1)));
    }
    if (rightValue) {
      this.state.rightValue.setValue(parseFloat(rightValue.toFixed(1)));
    }
  },
  ...
  render() {
    ...
    const {value, rightValue, containerSize, trackSize, thumbSize, allMeasured} = this.state;
    ...
    const offsetRight = rightValue.interpolate({
        inputRange: [minimumValue, maximumValue],
